#!/usr/bin/env python3
"""compute_hard_negatives_metrics.py

Process hard_negatives CSVs to:
1. Compute slope_deg and roughness_rms_m using the same algorithms as mission data
2. Label each point: 0 = too_rough.csv, 1 = too_tall.csv
3. Combine into one hard_negatives.csv with all metrics

Usage:
    python3 compute_hard_negatives_metrics.py
"""
import sys
...
OUT_PATH = SCRIPT_DIR / "hard_negatives.csv"
...
def process_file(path: Path, label: int):
    """Read, clean, compute metrics, and add label for one file."""
    print(f"\n{'='*70}")
    print(f"Processing: {path.name} (label={label})")
    print(f"{'='*70}")
...
def main():
    print(f"{'='*70}")
    print(f"HARD NEGATIVES METRICS COMPUTATION")
    print(f"{'='*70}")
